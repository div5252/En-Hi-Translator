...
{
  var alpha = List.generate(T, (i) => List.generate(n, (j) => 0.0), growable: false);
...
String _test_hmm(String filePath)
{
  ...
  List <double> amplitudes = fileContent.split('\n').map((e)=>double.parse(e)).toList();
...
List<List<double>> forward_procedure1(List pi, List a, List b, int n, int m, List o, int T)
{
  var alpha = List.generate(T, (i) => List.generate(n, (j) => 0.0), growable: false);
...
List<List<double>> backward_procedure(List pi, List a, List b, int n, int m, List o, int T)
{
  var beta = List.generate(T, (i) => List.generate(n, (j) => 0.0), growable: false);
	for(int j=0; j<n; j++)
	{
		beta[T-1][j]=1.0;
	}
...
{
  var gamma = List.generate(T, (i) => List.generate(n, (j) => 0.0), growable: false);
	for(int i=0; i<T; i++)
	{
		double sum=0.0;
		for(int j=0; j<n;j++)
...
List<List<List<double>>> get_ita(List alpha, List beta, List a, List b, List o, int n, int m, int T)
{
  var ita = List.generate(T, (i) => List.generate(n, (j) => List.generate(n, (k) => 0.0)), growable: false);
	for(int i=0; i<T-1; i++)
	{
		double sum=0.0;
		for(int j=0;j<n;j++)
		{
			for(int k=0; k<n;k++)
			{
				ita[i][j][k]=alpha[i][j]*a[j][k]*b[k][o[i+1]-1]*beta[i+1][k];
				sum+=ita[i][j][k];
			}
		}
...
{
	var gamma = List.generate(T, (i) => List.generate(n, (j) => 0), growable: false);
  var delta = List.generate(T, (i) => List.generate(n, (j) => 0.0), growable: false);
...
List<List<double>> update_state_transition_matrix(List ita, List gamma, int n, int T, List<List<double>> a)
{
...
List<List<double>> update_beta(List gamma, List o, int n, int m, int T, List<List<double>> b)
{
	for(int j=0; j<n;j++)
	{
		for(int k=0; k<m;k++)
...
void train_HMM(String filePath, String label)
{
  ...
  var piFileContent = piFile.readAsStringSync();
  List<double> pi = piFileContent.split(' ').map((e)=>double.parse(e)).toList();
  ...
  int m=b[0].length;
	...
	List<List<double>> alpha = forward_procedure1(pi, a, b, n, m, observation_sequence, T);
	List<List<double>> beta =	backward_procedure(pi, a, b, n, m, observation_sequence, T);
	List<List<double>> gamma = get_gamma(alpha, beta, n, T);
	List<List<List<double>>> ita = get_ita(alpha, beta, a, b, observation_sequence, n, m, T);
			
	List<int> optimal_state_sequence = viterbi(pi, a, b, n, m, observation_sequence, T);
